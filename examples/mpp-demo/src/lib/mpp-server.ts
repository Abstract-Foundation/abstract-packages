...
function getMppx() {
  ...
  const PAYMASTER_ADDRESS = process.env.PAYMASTER_ADDRESS as
    | `0x${string}`
    | undefined;
...
export function chargeHandler(request: Request) {
  const handler = payment(
    (getMppx() as Record<string, unknown>)["abstract/charge"] as Parameters<
      typeof payment
    >[0],
    {
      amount: CHARGE_AMOUNT,
...
export function sessionHandler(request: Request) {
  const handler = payment(
    (getMppx() as Record<string, unknown>)["abstract/session"] as Parameters<
      typeof payment
    >[0],
    {
      amount: SESSION_AMOUNT,
