...
export const sessionNamespaceDefinition: AgwCommandDefinition = defineCommand({
  ...
  children: [
    defineCommand({
      ...
      kind: "command",
      description:
        "Return the current AGW session status and delegated capability summary.",
      status: "implemented",
      inputMode: "json",
      ...
      exposure: exposure(true, true, [
        "request only the session fields required for the current reasoning step",
      ]),
      config: config({
        env: "AGW_HOME",
    ...
    defineCommand({
      ...
      kind: "command",
      description:
        "Run local health checks for AGW session, config, and companion connectivity.",
      status: "implemented",
      inputMode: "json",
      auth: authSession("missing_ok"),
