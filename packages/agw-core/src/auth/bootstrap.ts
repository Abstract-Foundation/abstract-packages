...
export async function runBootstrapFlow(
  ...
  if (previousSession && previousSession.status === "active") {
    logger.warn(
      "Replacing existing active session. The previous signer will remain registered until explicitly revoked.",
    );
  }
  ...
  try {
    ...
    logger.info(
      "Waiting for hosted session approval in the browser (callback window: 15 minutes)...",
    );
    ...
    if (bundle.state !== callbackState) {
      throw new Error(
        "Signer bundle state does not match the bootstrap request.",
      );
    }
    assertBundleMatchesBootstrapRequest({
      bundle,
    ...
    if (bundle.signerFingerprint !== expectedFingerprint) {
      throw new Error(
        `Signer fingerprint (${bundle.signerFingerprint}) does not match the local device key.`,
      );
    }

    const authKeyfilePath = storage.path.replace(
      /session\.json$/,
      "privy-auth.key",
    );
