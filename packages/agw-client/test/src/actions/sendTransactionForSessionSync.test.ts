...
vi.mock("../../../src/actions/sendTransactionInternal", () => ({
  sendTransactionInternal: vi.fn().mockResolvedValue({
    blockHash: "0x1234",
    blockNumber: 1n,
...
describe("sendTransactionForSessionSync", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("should call sendTransactionInternal with correct arguments and sync callback (function call)", async () => {
    vi.mocked(isSmartAccountDeployed).mockResolvedValue(true);
    await sendTransactionForSessionSync(
      baseClient,
      signerClient,
      publicClient,
      {
  ...
  it("should throw if to field is not set", async () => {
    vi.mocked(isSmartAccountDeployed).mockResolvedValue(true);
    await expect(
