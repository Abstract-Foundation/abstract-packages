  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Missing required fields: walletId, method, caip2, params, authorizationSignature.",
        },
        { status: 400 },
      );
    }
    if (!ALLOWED_METHODS.has(body.method)) {
      return NextResponse.json(
        { error: `Unsupported RPC method: ${body.method}.` },
        { status: 400 },
      );
    }
    ...
    if (!appId || !appSecret) {
      return NextResponse.json(
        { error: "Missing Privy server credentials." },
        { status: 500 },
      );
    }

    const response = await fetch(
      `${PRIVY_API_BASE}/v1/wallets/${encodeURIComponent(body.walletId)}/rpc`,
      {
        method: "POST",
        headers: {
