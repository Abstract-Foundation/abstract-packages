...
export function useSessionWizardState() {
  ...
  const storedSignerAddress = useSessionWizardStore(
    (state) => state.signerAddress,
  );
  const dangerAcknowledged = useSessionWizardStore(
    (state) => state.dangerAcknowledged,
  );
  ...
  const provisionedSigner = useSessionWizardStore(
    (state) => state.provisionedSigner,
  );
  const existingSigners = useSessionWizardStore(
  ...
  const setDangerAcknowledged = useSessionWizardStore(
    (state) => state.setDangerAcknowledged,
  );
  const setValidationError = useSessionWizardStore(
    (state) => state.setValidationError,
  );
  const proceedToCreating = useSessionWizardStore(
    (state) => state.proceedToCreating,
  );
  const backToPolicySelection = useSessionWizardStore(
    (state) => state.backToPolicySelection,
  );
  const markCreationSuccess = useSessionWizardStore(
    (state) => state.markCreationSuccess,
  );
  const markCreationError = useSessionWizardStore(
    (state) => state.markCreationError,
  );
  const retryResolution = useSessionWizardStore(
    (state) => state.retryResolution,
  );
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const derivedAgwAddress = await getSmartAccountAddressFromInitialSigner(
          signerAddress,
          publicClient,
        );
        const agwRegistered = await isAGWAccount(
          publicClient,
          derivedAgwAddress,
        );
        if (cancelled) return;
  ...
  return {
    ...
    retryResolution,
    isConnected: Boolean(
      ready && authenticated && agwAddress && storedSignerAddress,
    ),
  };
}
