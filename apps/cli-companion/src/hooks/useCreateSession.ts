...
export function useCreateAgentSigner() {
  ...
  const createAgentSigner = useCallback(
    async ({
      ...
      authPublicKey,
      callbackUrl,
    }: CreateAgentSignerInput): Promise<{
      redirectUrl: string;
      ...
      try {
        ...
        const provisionBody =
          (await provisionResponse.json()) as ProvisionedSignerResult & {
            error?: string;
          };
        if (!provisionResponse.ok) {
          throw new Error(
            provisionBody.error ?? "Failed to provision AGW CLI signer.",
          );
        }
        ...
        if (!finalizeResponse.ok || !finalized.redirectUrl) {
          throw new Error(
            finalized.error ??
              "Failed to finalize AGW CLI signer provisioning.",
          );
        }
