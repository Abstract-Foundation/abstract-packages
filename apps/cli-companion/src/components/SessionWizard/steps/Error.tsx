...
export default function ErrorStep() {
  const { error, backToPolicySelection, retryResolution } =
    useSessionWizardState();
  const isResolutionError =
    error?.includes("Abstract Global Wallet") || error?.includes("verify AGW");
  ...
  return (
    <div className={styles.wrapper}>
      <Card>
        <CardHeader>
          <CardTitle>
            {isResolutionError ? "Wallet Not Found" : "Delegation Failed"}
          </CardTitle>
        ...
        <CardFooter className={styles.footer}>
          <Button
            ...
            variant="primary"
            onClick={
              isResolutionError ? retryResolution : backToPolicySelection
            }
          >
            Try Again
          </Button>
