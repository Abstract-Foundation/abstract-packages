...
function validateCallPolicy(policy: unknown, index: number): void {
  ...
  if (
    ...
    !ADDRESS_PATTERN.test(policy.target)
  ) {
    throw new Error(
      `callPolicies[${index}].target must be a 20-byte 0x-prefixed hex address.`,
    );
  }

  if (
    policy.selector !== undefined &&
    (typeof policy.selector !== "string" ||
      !SELECTOR_PATTERN.test(policy.selector))
  ) {
    throw new Error(
      `callPolicies[${index}].selector must be a 4-byte 0x-prefixed hex selector.`,
    );
  }
}
...
function validateTransferPolicy(policy: unknown, index: number): void {
  ...
  validateAllowedKeys(
    policy,
    ["target", "maxValuePerUse"],
    `transferPolicies[${index}]`,
  );
  ...
  if (
    ...
    throw new Error(
      `transferPolicies[${index}].target must be a 20-byte 0x-prefixed hex address.`,
    );
  }

  validateUint(
    policy.maxValuePerUse,
    `transferPolicies[${index}].maxValuePerUse`,
  );
}
...
function validateToolList(value: unknown, fieldName: string): void {
  ...
  for (const [index, tool] of value.entries()) {
    if (
      typeof tool !== "string" ||
      !(ALL_SESSION_TOOLS as string[]).includes(tool)
    ) {
...
export function validateSessionPolicyConfig(
...
export function validateSessionPolicyMeta(policyMeta: SessionPolicyMeta): void {
  ...
  if (
    typeof policyMeta.presetId !== "string" ||
    !PRESET_IDS.has(policyMeta.presetId as SessionPolicyPresetId)
  ) {
    throw new Error("policyMeta.presetId is invalid.");
  }

  if (
    typeof policyMeta.presetLabel !== "string" ||
    policyMeta.presetLabel.trim() === ""
  ) {
  ...
  if (
    !Array.isArray(policyMeta.selectedAppIds) ||
    !policyMeta.selectedAppIds.every((entry) => typeof entry === "string")
  ) {
    throw new Error("policyMeta.selectedAppIds must be a string array.");
  }
  if (
    !Array.isArray(policyMeta.selectedContractAddresses) ||
    !policyMeta.selectedContractAddresses.every(
      (entry) => typeof entry === "string" && ADDRESS_PATTERN.test(entry),
    )
  ) {
    throw new Error(
      "policyMeta.selectedContractAddresses must be an address array.",
    );
  }
  if (
  ...
  if (
    !Array.isArray(policyMeta.warnings) ||
    !policyMeta.warnings.every((entry) => typeof entry === "string")
  ) {
    throw new Error("policyMeta.warnings must be a string array.");
  }
  if (
    !Number.isInteger(policyMeta.generatedAt) ||
    policyMeta.generatedAt <= 0
  ) {
    throw new Error(
      "policyMeta.generatedAt must be a positive unix timestamp in seconds.",
    );
  }
}

export function validateSessionPolicyPresetTemplate(
  ...
  if (
    ...
    !PRESET_IDS.has(template.id as SessionPolicyPresetId)
  ) {
    throw new Error("template.id is invalid.");
  }
  if (template.customMode !== false) {
    throw new Error(
      "template.customMode must be false for built-in templates.",
    );
  }
  ...
  if (typeof template.requiresDangerAcknowledgement !== "boolean") {
    throw new Error(
      "template.requiresDangerAcknowledgement must be a boolean.",
    );
  }
  ...
  validateUint(
    template.defaultLimits.feeLimit,
    "template.defaultLimits.feeLimit",
  );
  validateUint(
    template.defaultLimits.maxValuePerUse,
    "template.defaultLimits.maxValuePerUse",
  );
  if (
    ...
    throw new Error(
      `template.defaultLimits.expiresInSeconds must be between ${POLICY_MIN_EXPIRY_SECONDS} and ${POLICY_MAX_EXPIRY_SECONDS}.`,
    );
  }
...
function parseSessionPolicyConfig(value: unknown): SessionPolicyConfig {
  ...
  validateAllowedKeys(
    value,
    ["feeLimit", "maxValuePerUse", "callPolicies", "transferPolicies"],
    "Invalid custom policy sessionConfig",
  );
  ...
  if (!Array.isArray(callPoliciesRaw) || !Array.isArray(transferPoliciesRaw)) {
    throw new Error(
      "Invalid custom policy sessionConfig. callPolicies and transferPolicies must be arrays.",
    );
  }

  const callPolicies: SessionCallPolicy[] = callPoliciesRaw.map(
    (entry, index) => {
      ...
      validateAllowedKeys(
        entry,
        ["target", "selector"],
        `Invalid custom policy callPolicies[${index}]`,
      );
      ...
      if (entry.selector !== undefined) {
        if (typeof entry.selector !== "string") {
          throw new Error(
            `Invalid custom policy callPolicies[${index}].selector.`,
          );
        }
  ...
  const transferPolicies: SessionTransferPolicy[] = transferPoliciesRaw.map(
    (entry, index) => {
      if (
        !isRecord(entry) ||
        typeof entry.target !== "string" ||
        typeof entry.maxValuePerUse !== "string"
      ) {
      ...
      validateAllowedKeys(
        entry,
        ["target", "maxValuePerUse"],
        `Invalid custom policy transferPolicies[${index}]`,
      );
  ...
  if (typeof feeLimit !== "string" || typeof maxValuePerUse !== "string") {
    throw new Error(
      "Invalid custom policy sessionConfig. feeLimit and maxValuePerUse must be strings.",
    );
  }
...
export function parseCustomTemplateInput(value: unknown): {
  ...
  validateAllowedKeys(
    value,
    ["expiresInSeconds", "sessionConfig", "policyMeta"],
  ...
  return {
    ...
    policyMeta:
      value.policyMeta === undefined
        ? undefined
        : parseSessionPolicyMeta(value.policyMeta),
  };
}

function normalizeCustomMeta(
  policyMeta: SessionPolicyMeta | undefined,
): SessionPolicyMeta {
  if (policyMeta) {
    return policyMeta;
  }
...
export function parseCustomPolicyTemplate(rawInput: string): {
  ...
  try {
    ...
    throw new Error(
      `Invalid custom policy: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
...
function normalizeGuidedDraft(
  ...
  if (presetId === "custom") {
    throw new Error(
      "Guided draft normalization does not support custom preset.",
    );
  }

  const defaults = BUILT_IN_POLICY_PRESETS[presetId].defaultLimits;
  const selectedAppIds = guidedDraft?.selectedAppIds ?? [];
  const transferTargets = parseTransferTargets(
    guidedDraft?.transferTargets ?? [],
  );
  const expiresInSeconds =
    guidedDraft?.expiresInSeconds ?? defaults.expiresInSeconds;
  ...
  if (
    expiresInSeconds < POLICY_MIN_EXPIRY_SECONDS ||
    expiresInSeconds > POLICY_MAX_EXPIRY_SECONDS
  ) {
    throw new Error(
      `expiresInSeconds must be between ${POLICY_MIN_EXPIRY_SECONDS} and ${POLICY_MAX_EXPIRY_SECONDS}.`,
    );
  }
...
export function getPolicyPreview(options: {
  ...
  const mode =
    options.policyMode ??
    (options.presetId === "custom" ? "advanced" : "guided");

  if (options.presetId === "custom" || mode === "advanced") {
    const customInput =
      options.customPolicyJson?.trim() || buildDefaultCustomTemplateJson();
