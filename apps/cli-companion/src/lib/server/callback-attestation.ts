...
function loadConfiguredSigner(): CallbackSigner | null {
  ...
  const configuredPublicKey =
    process.env.AGW_CALLBACK_SIGNING_PUBLIC_KEY?.trim();
  if (configuredPublicKey && configuredPublicKey !== publicKeyBase64) {
    throw new Error(
      "Configured callback signing public key does not match the provided private key.",
    );
  }
...
function getCallbackSigner(): CallbackSigner {
  ...
  if (process.env.NODE_ENV === "production") {
    throw new Error(
      "Missing AGW_CALLBACK_SIGNING_PRIVATE_KEY in production environment.",
    );
  }
...
export function getCallbackSigningPublicKey(): {
  issuer: string;
  publicKey: string;
} {
...
export function signCallbackPayload<TPayload extends SignedCallbackClaims>(
  payload: TPayload,
): string {
  const signer = getCallbackSigner();
  const header = {
...
export function buildSignedCallbackPayload<
  TPayload extends Omit<SignedCallbackClaims, "iss" | "iat" | "exp">,
>(payload: TPayload, ttlSeconds = 300): TPayload & SignedCallbackClaims {
  const signer = getCallbackSigner();
...
export function verifySignedServerPayload<TPayload>(token: string): TPayload {
  ...
  if (parsed.header.alg !== "EdDSA") {
    throw new Error(
      `Unsupported signed token algorithm (${String(parsed.header.alg)}).`,
    );
  }
