...
function parseSharedParams(searchParams: URLSearchParams):
  ...
  if (!isLoopbackCallbackUrl(callbackUrl)) {
    return {
      error:
        "Invalid callback_url. Use a loopback URL (localhost, 127.0.0.1, or [::1]).",
    };
  }
  try {
...
export function parseRevokeParams(
  searchParams: URLSearchParams,
): RevokeValidationResult {
  const shared = parseSharedParams(searchParams);
  if ("error" in shared) {
    return {
      ok: false,
  ...
  if (!walletId || !signerId || !accountAddress) {
    return {
      ok: false,
      error:
        "Missing required parameters: account_address, wallet_id, signer_id.",
    };
  }
