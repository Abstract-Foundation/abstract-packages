import {
  HIGH_RISK_EXPIRY_SECONDS,
  HIGH_RISK_MAX_VALUE_PER_USE,
} from "./config";
...
export function assessPolicyRisk(preview: PolicyPreview): SecurityAssessment {
  ...
  if (lifetime > 12 * 60 * 60) {
    score += 1;
    reasons.push(
      "Session expiry exceeds 12h and may be too broad for unattended workflows.",
    );
  }
  ...
  if (
    policyPayload.sessionConfig.callPolicies.some((policy) => !policy.selector)
  ) {
    score += 1;
  ...
  if (
    enabledTools.has("sign_message") ||
    enabledTools.has("sign_transaction")
  ) {
    score += 1;
  ...
  if ((policyPayload.policyMeta?.unverifiedAppIds.length ?? 0) > 0) {
    score += 1;
    reasons.push(
      "One or more selected apps have unverified contract mappings.",
    );
  }
